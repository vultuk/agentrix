...
1. **Add launch option** – Update `WorktreeCommandAction` and the terminal launch sheets to include a
   “Codex SDK” action that triggers the new `/api/codex-sdk/session` endpoint instead of a terminal command.
2. **Chat transport** – Add a small client for `POST /api/codex-sdk/session` plus the
