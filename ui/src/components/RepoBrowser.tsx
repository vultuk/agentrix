...
import { REPOSITORY_POLL_INTERVAL_MS, SESSION_POLL_INTERVAL_MS, ORGANISATION_COLLAPSE_STORAGE_KEY } from '../config/constants.js';
...
import { useTerminalManagement } from '../hooks/useTerminalManagement.js';
import { useSessionManagement } from '../hooks/useSessionManagement.js';
import { useGitSidebar } from '../hooks/useGitSidebar.js';
...
import { useWorktreeOperations } from '../hooks/useWorktreeOperations.js';
import { usePlanManagement } from '../hooks/usePlanManagement.js';
import { useDiffManagement } from '../hooks/useDiffManagement.js';
...
export default function RepoBrowser({ onAuthExpired, onLogout, isLoggingOut }: RepoBrowserProps = {}) {
  ...
  // Helper function for worktree keys
  const getWorktreeKey = useCallback((org: string, repo: string, branch: string) => `${org}::${repo}::${branch}`, []);
  ...
  useEffect(() => {
    menus.clearActionMenuOnPendingClear(pendingWorktreeAction);
  }, [pendingWorktreeAction, menus]);
  ...
  const worktreeSelection = useWorktreeSelection({
    ...
    setOpenActionMenu: menus.setOpenActionMenu,
  ...
  const actionButtons = ActionBar({
    ...
    activeRepoDashboard,
    tasks,
    isTaskMenuOpen: menus.isTaskMenuOpen,
    hasRunningTasks,
    taskMenuRef: menus.taskMenuRef,
    planModalOpen: modals.planModal.open,
    planModalLoading: modals.planModal.loading,
    isGitSidebarOpen,
    onToggleTaskMenu: menus.toggleTaskMenu,
  ...
  const sidebar = h(Sidebar, {
    ...
    toggleActionMenu: menus.toggleActionMenu,
